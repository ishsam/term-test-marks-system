    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <div class="mb-3 row">
        <div class="col-sm-3">
          ...
          <?php
          $student_query = mysqli_query($db_con, "SELECT `tbl_student`.`id`, `tbl_student`.`first_name`,`tbl_student`.`last_name` FROM `tbl_student` INNER JOIN `tbl_student_subject` ON `tbl_student`.`class_id` = '$class' AND `tbl_student_subject`.`student_id` = `tbl_student`.`id` AND `tbl_student_subject`.`subject_id` = '$subject_id';");
                ...
                while ($student_result = mysqli_fetch_array($student_query)) {
                  ...
                  <input type="number" class="form-control" id="marks-'.$student_result['id'] .'" name="marks[]" placeholder="Marks">
                  </div>
